  ...
  axios.post('/api/eats', {
    ...
    <div class="card-body">
      ...
      <div class="cardActions d-flex justify-content-between">
        <svg class="bi bi-trash" onclick="deleteEats(this.dataset.id)" data-id=${data.id} width="1.3em" height="1.3em" viewBox="0 0 16 16" fill="currentColor"
        ...
        <svg class="bi bi-pencil-square" onclick="editEats()" data-id=${data.id} width="1.3em" height="1.3em" viewBox="0 0 16 16" fill="currentColor"
          xmlns="http://www.w3.org/2000/svg">
          <path
...
let deleteEats = function (id) {
  axios.delete(`/api/eats/${id}`)
    .then(() => { document.getElementById(`eat-${id}`).remove() })
    .catch(err => console.error(err))
}
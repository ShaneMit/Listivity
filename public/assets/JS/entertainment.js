document.getElementById('entertainmentAdd').addEventListener('click', event => {
  ...
  axios.post('/api/entertains', {
    ...
    .then(({ data }) => {
      ...
      entertainmentElem.className = 'card d-flex flex-column justify-content-center'
      entertainmentElem.id = `entertainment-${data.id}`
      ...
      <div class="card-body">
        ...
        <div class="cardActions d-flex justify-content-between">
          ...
          <svg data-toggle="modal" data-target="#editModalCenter" class="bi bi-pencil-square" onclick="editEntertainment()" data-id=${data.id} width="1.3em" height="1.3em" viewBox="0 0 16 16" fill="currentColor"
            xmlns="http://www.w3.org/2000/svg">
            <path
...
let deleteEntertainment = function (id) {
  axios.delete(`/api/entertains/${id}`)
    .then(() => { document.getElementById(`entertainment-${id}`).remove() })
    .catch(err => console.error(err))
}